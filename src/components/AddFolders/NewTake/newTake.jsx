...
export const AddTake = () => {
  ...
  const gamesMissing = useSelector((state) => state.gamesWithMissingPart.gamesWithMissingParts).data;
  const existingReturnIDs = useSelector((state) => state.takingOrReturning.takingsOrReturnings.map((item) => item.ReturnID));

  const closeAdd = () => navigate("/singleUser/Taking_Returning");

  const addTakeGame = () => setSelectedGames((prev) => [...prev, {}]);

  const selectGameName = (index) => (e) => {
    const selectedGameId = e.target.value;
    setSelectedGames((prev) => {
      const newSelectedGames = [...prev];
      newSelectedGames[index] = availableGames.find((game) => game.Id === selectedGameId);
  ...
  const ApprovalTake = async () => {
    ...
    const formattedReturnDate = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split("T")[0];

    const results = await Promise.all(selectedGames.map(async (game) => {
      const take = {
        IsMissingParts: game.CurrentStateOfGame !== "תקין",
      ...
      if (addResponse && !addResponse.err) {
        const updatedGame = { ...game, IsAvailable: "FALSE" };
        const updateGame = await UpdateGameTOR(updatedGame);
        if (updateGame) dispatch(UPDATE_GAME(updateGame));
    ...
    setCircleFlag(false);
    const failureMessages = results.filter(result => !result.success).map(result => result.name);
  ...
  const handleDeleteTakegame = (index) => {
    setSelectedGames((prev) => (prev.length === 1 ? [{}] : prev.filter((_, i) => i !== index)));
  };
  ...
  return (
    <div className="add-take-screen">
      <div className="nav-bar-AddTake">
        ...
        <div className="close-add" onClick={closeAdd}>x</div>
      </div>
      ...
      <div className="add-form">
        ...
        {selectedGames.map((selectedGame, index) => {
          const age = forAgesFromStore.find(a => a.AgeCode === selectedGame.AgeCode)?.Age;
          const tchum = typesGamesFromStore.find(t => t.gameTypeCode === selectedGame.GameTypeCode)?.gameTipeName;
          const missP = selectedGame.CurrentStateOfGame === "חסרים חלקים" ? gamesMissing.find(g => g.Id === selectedGame.Id) : null;

          return (
            <div key={index}>
              <div className="new-hashala">
                <div style={{ width: "80vw", display: "inline-flex" }}>
                  <select onChange={selectGameName(index)} className="select-game-take">
                    <option value="" disabled selected>בחר משחק</option>
                  ...
                  {selectedGame && (
                    <>
                      <div className="status-row">{selectedGame.CurrentStateOfGame}</div>
                      <div className="ages-row">{age}</div>
                      <div className="tchum-row">{tchum}</div>
                      <div className="place-row">{`${selectedGame.ClosetNumber} ${selectedGame.Location}`}</div>
                      <div className="Complementar-Available-take">
                        {selectedGame.HaveComplementaryGame === "on" || selectedGame.HaveComplementaryGame === "TRUE" ? (
                          <div className="Complementar-take"><div className="v-logo-take"></div>קיים משחק משלים</div>
                        ) : null}
                        {selectedGame.IsAvailable === "on" || selectedGame.IsAvailable === "TRUE" ? (
                          <div className="Available-take"><div className="v-logo-take"></div>זמין להשאלה</div>
                        ) : null}
                      </div>
                    </>
                  )}
                </div>
                <div className="garb-icon-take" onClick={() => handleDeleteTakegame(index)}></div>
              </div>
              {selectedGame && (
                <div style={{ display: "inline-flex", borderBottom: "1px rgba(230, 230, 230, 1) solid", width: "90vw", marginRight: "5vw" }}>
                  {selectedGame.Parts && Array.isArray(selectedGame.Parts) && (
                  ...
                  {selectedGame.CurrentStateOfGame === "חסרים חלקים" && missP && missP.MissingParts && (
                    <div className="missParts">
                      <div style={{ display: "block" }}>
                        <div className="game-parts-name-take">חלקים חסרים במשחק:</div>
        ...
        {selectedGames[0].GameName && (
          <button onClick={ApprovalTake} className="Approval-take-btn">
            {circleFlag ? <CircularProgress size={24} color="white" /> : status === "success" ? <CheckIcon /> : status === "error" ? <CloseIcon /> : "אישור"}
          </button>
        )}
      </div>
    </div>
