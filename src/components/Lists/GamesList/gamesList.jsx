...
// import { Button, AccordionDetails, AccordionSummary, Accordion } from "@mui/material";
...
//   const searchNames = [{ name: "תחום" }, { name: "טווח גילאים" }, { name: "מס’ ארון" }];
//   const navigate = useNavigate();
//   const dispatch = useDispatch();

//   const { games, users, typesGames, closets, tor, gamesMissing, forAges } = useSelector((state) => ({
//     games: state.game.games,
//     users: state.user.users.data,
//     typesGames: state.typeGame.typesGames,
//     closets: state.closet.closets,
//     tor: state.takingOrReturning.takingsOrReturnings,
//     gamesMissing: state.gamesWithMissingPart.gamesWithMissingParts.data,
//     forAges: state.forAge.forAges.data,
//   }));
...
//         const filterMap = {
//           "תחום": typesGames.filter((t) => filters[filter].includes(t.gameTipeName)).map((t) => t.gameTypeCode),
//           "טווח גילאים": forAges.filter((a) => filters[filter].includes(a.Age)).map((a) => a.AgeCode),
//           "מס’ ארון": closets.filter(c => filters[filter].includes(c.closetCode)).map(c => c.closetCode),
//         };
//         filteredGames = filteredGames.filter((game) => filterMap[filter]?.includes(game[`${filter}Code`]));
//       }
//     });

//     return filteredGames;
//   }, [games, filters, typesGames, forAges, closets]);

//   useEffect(() => {
//     setTableArr(filterGames());
//   }, [filterGames]);

//   const userName = (Id) => {
//     const gameTake = tor.filter((tg) => tg.GameCode === Id);
//     if (!gameTake.length) return "No user found";

//     const closestDateObject = gameTake.reduce((closest, current) => {
//       return new Date(current.TakingDate) < new Date(closest.TakingDate) ? current : closest;
//     });

//     return users.find((u) => u.userCode === closestDateObject.UserCode)?.userName || "Unknown User";
//   };
...
//     const color = typesGames.find((t) => t.gameTypeCode === game.GameTypeCode)?.stickerColor || "green";
//     stickerPDF({ code: game.Id, name: game.GameName }, color);
//   };

//   const exportToPDF = () => {
//     const columns = [
//       "קוד משחק", "קוד ארון", "שם משחק", "תחום משחק", "חלקי משחק",
//       "המשחק מיועד לגילאי", "מצב משחק", "מדבקה?", "משחק משלים?",
//       "מיקום הארון", "זמין להשאלה?", "מיקום בארון"
//     ];
//     const formattedData = formatNestedObjects(tableArr);
//     generatePDF(columns, formattedData, "משחקים");
//   };
...
//         <Button sx={{ width: "150px", height: "38px", top: "20px", borderRadius: "28px" }} variant="contained" onClick={NewGame}>
//           משחק +
//         </Button>
//       </div>
...
//           <div key={i}>
//             <SearchButtons name={search.name} list={games} onFilterChange={handleFilterChange} />
//           </div>
//         ))}
//       </div>
...
//           {tableArr.map((game, i) => {
//             const age = forAges.find((a) => a.AgeCode === game.AgeCode)?.Age;
//             const tchum = typesGames.find((t) => t.gameTypeCode === game.GameTypeCode)?.gameTipeName;
//             const missP = game.CurrentStateOfGame === "חסרים חלקים" && gamesMissing.find((g) => g.Id === game.Id);

//             return (
//               <div key={i}>
//                 <Accordion sx={{ direction: "rtl", borderBottom: "2px rgba(6, 120, 252, 0.20) solid" }}>
//                   <AccordionSummary sx={{ height: "15vh" }} expandIcon={<ExpandMoreIcon sx={{ color: "rgba(6, 120, 252, 1)" }} />}>
//                     <div className="game-column">
//                       <div className="game-details">
//                         <div className="game-names">{game.GameName}</div>
//                         <div className="game-names-second">
//                           <div className="age"><div className="child-logo"></div>{age}</div>
//                           <div className="pas">|</div>
//                           <div className="type"><div className="tchum">תחום:</div><div>{tchum}</div></div>
//                         </div>
//                       </div>
//                       <div className="status">
//                         <div className={`CurrentStateOfGame ${game.CurrentStateOfGame === "תקין" || !game.CurrentStateOfGame ? "green" : game.CurrentStateOfGame === "חסרים חלקים" ? "red" : ""}`}></div>
//                         <div>{game.CurrentStateOfGame || "תקין"}</div>
//                       </div>
//                       <div className="place">
//                         <div className="closet-logo"></div>
//                         <div className="mikum-in-closet">({game.PlaceInCloset})</div>
//                         <div className="closet-number">{game.ClosetNumber}</div>
//                         <div className="closet-place">{game.Location}</div>
//                       </div>
//                       <div className="game-mushal">
//                         {game.IsAvailable !== "FALSE" && game.IsAvailable !== "" ? (
...
//                         {game.HaveComplementaryGame === "TRUE" || game.HaveComplementaryGame === "true" ? (
//                           <div className="Complementar"><div className="v-logo"></div>קיים משחק משלים</div>
//                         ) : null}
//                         {game.IsAvailable === "TRUE" || game.IsAvailable === "true" ? (
//                           <div className="Available"><div className="v-logo"></div>זמין להשאלה</div>
//                         ) : null}
//                       </div>
//                       <div className="update-delete-icons">
//                         <div className="update-icon" onClick={() => navToNewGame(game)}></div>
//                         <div className="delete-icon" onClick={() => deleteAgame(game)}></div>
//                         <StyleIcon onClick={() => printAsticker(game)} className="sticker" sx={{ color: "rgba(6, 120, 252, 1)" }} />
//                       </div>
//                     </div>
//                   </AccordionSummary>
//                   <AccordionDetails>
...
//                                 <div>({part.amount}{part.amont})</div>
//                                 <div>{part.name}</div>
//                               </div>
//                             ))}
//                           </div>
//                         </div>
//                         <div className="sum">סה"כ:<div className="sum-number">({game.Parts.reduce((total, part) => total + (Number(part.amount) || Number(part.amont)), 0)})</div></div>
//                       </div>
//                       <div className="acordi-comment">{game.Comment}</div>
//                     </div>
//                     {game.CurrentStateOfGame === "חסרים חלקים" && missP && missP.MissingParts && (
//                       <div className="AccordionDetails-missParts">
//                         <div className="game-parts">
//                           <div style={{ display: "block" }}>
//                             <div className="game-parts-name">חלקים חסרים במשחק:</div>
//                             <div className="parts-table">
//                               {missP.MissingParts[0].rows.map((part, j) => (
//                                 part.amount - part.afterReturn > 0 && (
//                                   <div key={j} className="parts">
//                                     <div>({part.amount - part.afterReturn})</div>
//                                     <div>{part.name}</div>
//                                   </div>
//                                 )
//                               ))}
